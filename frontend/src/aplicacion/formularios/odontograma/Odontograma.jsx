...
export default function Odontograma({ patientId = 'demo', patientName = '', historyCode = '' }) {
  ...
  const run = (action) => { try { action(); } catch (e) { setError(e.message); } };
  ...
  const apply = (number = selectedTooth, nextSurface = surface, tracedPoints = []) => run(() => {
    ...
    if (!tracedPoints.length && existing.some((mark) => mark.state === stateId && mark.code === code && mark.condition === (state.condition ? condition : 'good') && mark.note === note.trim())) {
      setMessage('Este hallazgo ya está registrado en la selección.'); return;
    }
    ...
    if (persist(updateExamination(record, exam.id, { teeth: next.teeth, ranges: next.ranges }))) { setPoints([]); setNote(''); setReviewed(false); }
  });
  const select = (number, nextSurface) => {
    ...
    if (mode === 'erase') {
      ...
      if (mark) remove(mark.id);
      return;
    }
    apply(number, nextSurface);
  };
  const add = () => apply(selectedTooth, surface, points);
  const remove = (id) => run(() => { const next = removeFinding(exam, id); if (persist(updateExamination(record, exam.id, { teeth: next.teeth, ranges: next.ranges }))) setReviewed(false); });

  return <div className="undac-section-stack hc-odontogram nts-odontogram">
    <SectionCard title="Odontograma" subtitle="Seleccione un hallazgo y marque la pieza o superficie afectada.">
      ...
      <div className="nts-chart-scroll"><div className="nts-chart">
